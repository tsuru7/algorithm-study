...
def solve(n,ax,ay,bx,by,smap):
    ...
    dirx = [-1, 1, 1, -1]
    diry = [-1, -1, 1, 1]
    depth = [[[INFTY for _ in range(4)] for _ in range(n)] for _ in range(n)]
    ...
    while len(queue) > 0:
        ...
        if 0 <= vx < n and 0 <= vy < n and smap[vx][vy] == '.':
            for vdir in [ul, ur, dr, dl]:
                if vdir == udir:
                    cost = depth[ux][uy][udir]
                else:
                    cost = depth[ux][uy][udir] + 1
                if cost < depth[vx][vy][vdir]:
                    depth[vx][vy][vdir] = cost
                    if vdir == udir:
