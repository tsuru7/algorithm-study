...
def solve(H,W,n,h,w,amat):
    ...
    cum2d = [[[0 for _ in range(n+1)] for _ in range(W+1)] for _ in range(H+1)]
    for row in range(H):
        for col in range(W):
            aij = amat[row][col]
            count[aij] += 1
    ...
    for row in range(1, H+1):
        for col in range(1, W+1):
            ...
            for num in range(1, n+1):
                if num == aij:
                    cum2d[row][col][num] = cum2d[row][col-1][num] + 1
                else:
                    cum2d[row][col][num] = cum2d[row][col-1][num]
    for col in range(1, W+1):
        for row in range(1, H+1):
            for num in range(1, n+1):
                cum2d[row][col][num] += cum2d[row-1][col][num]
    # for num in range(1, n+1):
    ...
    for row in range(H-h+1):
        for col in range(W-w+1):
            ...
            tmp = 0
            for num in range(1, n+1):
                delcnt = cum2d[d][r][num] + cum2d[u-1][l-1][num] \
                       - cum2d[d][l-1][num] - cum2d[u-1][r][num]
