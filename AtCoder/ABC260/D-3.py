...
def solve(n,k,p):
    # slには(一番上のカード, 一番下のカード)のタプルで場にある山を格納する。
    # slには場にある山が一番上のカードの昇順に整列されて格納される。
    # yamaには一番下のカードをキーとして、それぞれの山に含まれるカードをリスト形式で格納する
    # eatはカードが食べられたターン数を格納する
    sl = SortedList()
    yama = dict()
    eat = [-1 for _ in range(n)]
    for i in range(n):
        pi = p[i]
        # idxはslの中に(pi, pi)を挿入する場所のインデックス
        # この問題では全ての要素の値が異なるのでsl[idx]が(pi, pi)より大きい最小の要素
        idx = sl.bisect_left((pi, pi))
        if idx == len(sl):
            top = pi
            base = pi
            sl.add((top, base))
